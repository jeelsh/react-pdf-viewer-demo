    ...
    return (
        <div className="w-full absolute bottom-0 bg-transparent z-50" style={{ paddingBottom: paddingBottom }}>
            <div className={`px-3 transition-all duration-300 ease-in-out ${!isMobileOrTablet && isIndexVisible ? 'ml-0' : 'ml-0'}`}>
                ...
                <div className="flex items-center gap-1 mb:gap-3 pb-3">
                    <div className="hidden lg:block flex-1"></div>
                    {/* Main navigation controls with glass effect */}
                    <div className="border border-white/20 rounded-full flex items-center gap-2 px-2 sm:px-4 py-1.5 sm:py-2 bg-background/80 backdrop-blur-md shadow-lg">
                        <Button
                        ...
                        >
                            <ChevronLeft className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
                        </Button>
                        <Button
                            ...
                            className='size-8 sm:size-9 min-w-8 sm:min-w-9 rounded-full hover:bg-white/10 disabled:opacity-30 transition-all'
                        >
                            <ChevronRight className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
                        </Button>
                        <div className="hidden md:block w-px h-5 sm:h-6 bg-white/20 mx-0.5 sm:mx-1"></div>
                        <Zoom zoomScale={viewerStates.zoomScale} screenWidth={screenWidth} />
                        <div className="w-px h-5 sm:h-6 bg-white/20 mx-0.5 sm:mx-1"></div>
                        ...
                        >
                            {screenfull.isEnabled && screenfull.isFullscreen ?
                                <Minimize className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" /> :
                                <Maximize className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
                            }
                        </Button>
                    </div>
...
function SpotifyControls() {
    ...
    if (!token) {
        return (
            <div className="border border-white/20 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 bg-background/80 backdrop-blur-md shadow-lg">
                <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={onLogin}
                    className="hover:bg-transparent text-foreground transition-all text-xs sm:text-sm h-7 sm:h-8"
                >
                    Login Spotify
                </Button>
            </div>
    ...
    return (
        <div className="border border-white/20 rounded-full flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-1.5 sm:py-2 bg-background/80 backdrop-blur-md shadow-lg">
            <Button 
                variant='ghost' 
                size='icon' 
                className='size-8 sm:size-9 min-w-8 sm:min-w-9 rounded-full hover:bg-white/10 disabled:opacity-30 transition-all' 
                ...
                disabled={!ready}
            >
                <SkipBack className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
            </Button>
            <Button 
                variant='ghost' 
                size='icon' 
            ...
            >
                {isPaused ? (
                    <Play className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
                ) : (
                    <Pause className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
                )}
            </Button>
            <Button 
                variant='ghost' 
                ...
                disabled={!ready}
            >
                <SkipForward className="size-4 sm:size-5 min-w-4 sm:min-w-5 text-foreground" />
            </Button>
            {track && (
                <>
                    <div className="w-px h-5 sm:h-6 bg-white/20"></div>
                    <div className='ml-1 max-w-[150px] sm:max-w-[220px] truncate text-[10px] sm:text-xs font-medium text-foreground'>
